...
// G.711 μ-law encoding/decoding (inline to avoid external dependency)
function encodePCMU(input: Buffer): Buffer {
  ...
  for (let i = 0, o = 0; i < input.length; i += 2, o++) {
    ...
    out[o] = ~(sign | (exponent << 4) | mantissa) & 0xff;
  }
  return out;
}

function decodePCMU(input: Buffer): Buffer {
  const out = Buffer.alloc(input.length * 2);
  for (let i = 0, o = 0; i < input.length; i++, o += 2) {
    let uVal = ~input[i] & 0xff;
...
export class RTPAudioHandler extends EventEmitter {
  ...
  async start(): Promise<void> {
    ...
    this.socket.on('message', (msg, rinfo) => {
      ...
      if (
        ...
        this.remotePort = rinfo.port;
        console.log(
          `[RTPAudioHandler] Remote endpoint detected: ${this.remoteAddress}:${this.remotePort}`
        );
      }
      ...
      if (audioData) {
        ...
        if (this.receivedPackets === 1) {
          console.log(`[RTPAudioHandler] First RTP packet received, size: ${msg.length}`);
        }
    ...
    this.socket.on('error', (error) => {
      console.error('[RTPAudioHandler] Socket error:', error);
      this.emit('error', error);
    });

    return new Promise((resolve, reject) => {
      this.socket!.bind(this.localPort, () => {
        console.log(`[RTPAudioHandler] Listening on port ${this.localPort}`);
        resolve();
      });
  ...
  setRemoteEndpoint(address: string, port: number): void {
    ...
    console.log(`[RTPAudioHandler] Remote endpoint set: ${address}:${port}`);
  }
  ...
  sendAudio(pcmData: Buffer): void {
    if (!this.socket || !this.remoteAddress || !this.remotePort) {
      console.warn('[RTPAudioHandler] Cannot send audio: RTP not connected');
      return;
    }
    ...
    this.socket.send(rtpPacket, this.remotePort, this.remoteAddress, (error) => {
      if (error) {
        console.error('[RTPAudioHandler] Failed to send RTP packet:', error);
      }
    });
  ...
  async stop(): Promise<void> {
    if (this.socket) {
      return new Promise<void>((resolve) => {
        this.socket!.close(() => {
          console.log('[RTPAudioHandler] Socket closed');
          this.socket = null;
          resolve();
        });
      });
