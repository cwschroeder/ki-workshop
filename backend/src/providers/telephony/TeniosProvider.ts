...
export class TeniosProvider implements ITelephonyProvider {
  ...
  async retrieveRecording(options: RetrieveRecordingOptions): Promise<RetrieveRecordingResponse> {
    ...
    console.log(`[TeniosProvider] Retrieving recording: ${options.recordingUuid}`);
    console.log(`[TeniosProvider] Request body:`, JSON.stringify(body));
    ...
    console.log(`[TeniosProvider] Response status: ${response.status}`);

    if (!response.ok) {
      const errorText = await response.text();
      console.error(`[TeniosProvider] Error response: ${errorText}`);
  ...
  /**
   * Parse Call HTTPS-Post webhook payload
   *
   * TENIOS sends these webhooks at various call events.
   * Configure the webhook URL in TENIOS portal under routing settings.
   *
   * Documentation: https://www.tenios.de/doc/api-call-posts
   *
   * Example TENIOS payload:
   * {
   *   "request_type": "CALL_END",
   *   "call_uuid": "abc123",
   *   "caller_id_number": "+49123456789",
   *   "destination_number": "+49987654321",
   *   "call_start": "2025-11-25T10:00:00Z",
   *   "call_end": "2025-11-25T10:05:00Z",
   *   "duration": 300,
   *   "hangup_cause": "NORMAL_CLEARING",
   *   "recording_uuid": "rec-xyz",
   *   "variables": { ... }
   * }
   */
  parseCallPost(body: any): CallPostData {
    // TENIOS uses "requestType" (camelCase) for the event type
    const requestType = body.requestType || body.request_type || body.type || 'UNKNOWN';

    // Map TENIOS request types to our CallPostType
    let type: CallPostType;
    switch (requestType) {
      case 'CALL_START':
        type = 'CALL_START' as CallPostType;
        break;
      case 'BLOCK_END':
        type = 'BLOCK_END';
        break;
      ...
      default:
        ...
        type = requestType as CallPostType;
    }

    return {
      type,
      callUuid: body.call_uuid || body.variables?.call_uuid || '',
      callControlUuid: body.callControlUuid || body.call_control_uuid,
      callerNumber: body.caller_id_number || body.variables?.caller_id_number,
      destinationNumber: body.destination_number || body.variables?.destination_number,
      callStart: body.call_start || body.variables?.call_start,
      callEnd: body.call_end,
      duration: body.duration ? parseInt(body.duration, 10) : undefined,
      hangupCause: body.hangup_cause || body.variables?.hangup_cause,
      recordingUuid: body.recording_uuid || body.variables?.recording_uuid,
      variables: body.variables || {},
      dtmfDigits: body.dtmf_digits || body.variables?.collected_user_input,
      raw: body
    };
  }
}
