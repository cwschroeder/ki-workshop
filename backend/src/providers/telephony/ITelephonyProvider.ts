...
/**
 * Call HTTPS-Posts API Types
 * Documentation: https://www.tenios.de/doc/api-call-posts
 *
 * These webhooks are sent by TENIOS at various call events.
 * Configure in TENIOS portal under routing settings.
 */
export type CallPostType =
  | 'CALL_START'       // When call starts
...
export interface CallPostData {
  /** Type of the call post event */
  type: CallPostType;
  /** Unique call identifier */
  callUuid: string;
  /** Call control UUID (routing session) */
  ...
  callStart?: string;
  /** Call end timestamp (ISO 8601) - only for CALL_END */
  callEnd?: string;
  /** Call duration in seconds - only for CALL_END */
  duration?: number;
  /** Hangup cause code */
  hangupCause?: string;
  /** Recording UUID if call was recorded */
  recordingUuid?: string;
  /** All call variables */
  variables?: Record<string, any>;
  /** DTMF digits collected during call */
  dtmfDigits?: string;
  /** Raw request body for provider-specific data */
  raw?: any;
}
