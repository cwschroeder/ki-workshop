...
export class AudioMixer extends EventEmitter {
  ...
  addLeg(legId: string, rtpHandler: RTPAudioHandler): void {
    if (this.legs.has(legId)) {
      logger.warn({ sessionId: this.sessionId, legId }, 'Leg already exists in mixer');
      return;
    }

    logger.info({ sessionId: this.sessionId, legId }, 'Adding leg to audio mixer');
    ...
    rtpHandler.on('audio', (audioData: Buffer) => {
      ...
      if (legId === 'legA') {
        packetCount++;
        if (packetCount % 50 === 0) {
          logger.debug({ sessionId: this.sessionId, legId, packets: packetCount, bytes: audioData.length }, '🔊 Mixer emitting legAAudio');
        }
        this.emit('legAAudio', audioData);
      } else if (legId === 'legB') {
        packetCount++;
        if (packetCount % 50 === 0) {
          logger.debug({ sessionId: this.sessionId, legId, packets: packetCount, bytes: audioData.length }, '🔊 Mixer emitting legBAudio');
        }
    ...
    logger.debug({ sessionId: this.sessionId, legId, totalLegs: this.legs.size }, 'Leg added to mixer');
  }
  ...
  removeLeg(legId: string): void {
    ...
    if (!leg) {
      logger.warn({ sessionId: this.sessionId, legId }, 'Leg not found in mixer');
      return;
    }

    logger.info({ sessionId: this.sessionId, legId }, 'Removing leg from audio mixer');
  ...
  stop(): void {
    ...
    logger.info({ sessionId: this.sessionId }, 'Stopping audio mixer');
